...
class AutoPartyFormation:
    ...
    def formation(self, full_roles:dict[Any,int], light_roles:dict[Any,int], trance:dict[Any, Any]) -> Tuple[list, list]:
        '''full_roles {ロール名 : 人数}
        ...
        count = 1
        while len(assigned_players) < len(self.players): # 参加プレイヤと振り分け済みプレイヤの数が同じになるまで
            party = {role: [] for role in full_roles} # パーティ枠
        ...
        # trance player roles
        for player in light_players.keys():
            role_list = []
            for role in light_players[player]:
                if role not in role_list or role not in trance.keys():
                    role_list.append(trance[role])
        ...
        while len(assigned_players) < len(light_players):
            ...
            for role in light_roles: # role:user
                if role not in self.role_preference:
                    print(f'add preference {role}')
                    self.role_preference[role] = []
                for player in self.role_preference[role]: # player:list[user] role_preferenceにroleが含まれていないことがある
